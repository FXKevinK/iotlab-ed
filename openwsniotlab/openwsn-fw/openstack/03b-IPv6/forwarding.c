...
void forwarding_receive(
    ...
    if (
        ...
        uint8_t queue = openqueue_getUsedQueue();
        // if(queue > QUEUELENGTH / 2){
        //     LOG_INFO(COMPONENT_FORWARDING, ERR_DEBUG_1, queue, QUEUELENGTH);
        // }

        if (openqueue_isHighPriorityEntryEnough(TRUE) == FALSE) {
            ...
            LOG_WARNING(COMPONENT_FORWARDING, ERR_FORWARDING_PACKET_DROPPED, queue, msg->length);
            openqueue_freePacketBuffer(msg);
            return;
        }
