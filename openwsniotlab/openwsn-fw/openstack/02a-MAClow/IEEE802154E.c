    ...
    */
    ...
    { // this "loop" is only executed once
        ...
        if (ieee154e_vars.dataReceived->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC)
        {
            ...
            { // discard other frames as we cannot decrypt without being synced
                break;
            }
        }
...
port_INLINE void activity_ti1ORri1(void)
{
    ...
    case CELLTYPE_TX:
...
#if use_qtrickle == TRUE
        if (schedule_isMinimalCell() && opentrickletimers_isInTRange() && ieee154e_vars.dataToSend != NULL && is_dio == FALSE)
        {
            temp = opentrickletimers_get_p(0) / (float)(float_multiplier * 100.0);
            if (packetfunctions_random_p(temp))
            {
                ieee154e_vars.dataToSend = NULL;
            }
        }
        ...
        else
        {
            ...
            if (couldSendEB == TRUE)
            { // I will be sending an EB copy synch IE -- should be Little endian?
            ...
            if (ieee154e_vars.localCopyForTransmission.l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC)
            { // security enabled
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime)
{
    ...
    {
        // log the error
        LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)1, (errorparameter_t)0);
        // abort
        endSlot();
        return;
    }
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime)
{
    ...
    {
        // log the error
        LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)2, (errorparameter_t)0);
        // abort
        endSlot();
        return;
    }
    ...
    { // this "loop" is only executed once
        ...
        if (ieee154e_vars.dataReceived->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC)
        {
            ...
            else if (ieee154e_vars.dataReceived->l2_frameType == IEEE154_TYPE_BEACON)
            { // not joined yet
        ...
        if (ieee802514_header.ackRequested == 1 && ieee154e_vars.isAckEnabled == TRUE)
        {
            ...
            {
                // log the error
                LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)3, (errorparameter_t)0);
...
port_INLINE void activity_ri6(void)
{
    ...
    {
        // log the error
        LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)4, (errorparameter_t)0);
        // indicate we received a packet anyway (we don't want to loose any)
        notif_receive(ieee154e_vars.dataReceived);
        // free local variable
        ieee154e_vars.dataReceived = NULL;
...
void ieee154e_setSlotDuration(uint16_t newSlotDuration)
{
    ieee154e_vars.slotDuration = newSlotDuration;
}
