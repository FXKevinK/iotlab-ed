...
OpenQueueEntry_t* openqueue_getFreePacketBuffer(uint8_t creator) {
    ...
    if (openqueue_isHighPriorityEntryEnough(FALSE) == FALSE && creator > COMPONENT_SIXTOP_RES) {
        ENABLE_INTERRUPTS();
        return NULL;
    }
...
bool openqueue_isHighPriorityEntryEnough(bool print_if_false) {
    uint8_t i;
    uint8_t highpriorEntry;
    INTERRUPT_DECLARATION();
    DISABLE_INTERRUPTS();
    ...
    if (highpriorEntry > QUEUELENGTH - HIGH_PRIORITY_QUEUE_ENTRY) {
        // if(print_if_false){
        //     for (i = 0; i < HIGH_PRIORITY_QUEUE_ENTRY; i++) {
        //         if (openqueue_vars.queue[i].creator != COMPONENT_NULL || openqueue_vars.queue[i].owner != COMPONENT_NULL) {
        //             LOG_ERROR(COMPONENT_OPENQUEUE, ERR_DEBUG_1, openqueue_vars.queue[i].creator, openqueue_vars.queue[i].length);
        //         }
        //     }
        // }
        ENABLE_INTERRUPTS();
...
uint8_t openqueue_getUsedQueue(void) {
    uint8_t i;
