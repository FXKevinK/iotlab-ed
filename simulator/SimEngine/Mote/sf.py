...
class SchedulingFunctionMSF(SchedulingFunctionBase):
    ...
    def stop(self):
        for handle in [self.SLOTFRAME_HANDLE_AUTONOMOUS_CELLS,
                       self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS]:
            slotframe = self.mote.tsch.get_slotframe(handle)
            cells = []
            for _, _cells in slotframe.slots.items():
                cells += _cells
            for cell in cells:
                slotframe.delete(cell)
            self.mote.tsch.delete_slotframe(handle)
